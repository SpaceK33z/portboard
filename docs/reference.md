...
Configuration, command behavior, and implementation details for Portboard.

- [Project configuration](#project-configuration)
- [Standalone commands](#standalone-use)
- [Local dashboard](#local-dashboard)
- [Herdr integration](#herdr-integration)
- [Process and endpoint discovery](#process-and-endpoint-discovery)
...
For example, one `pnpm dev` launch target can be represented by a parent process plus Vite and API child processes. Its metadata names the primary `web` URL and direct `api` URL while generic listener discovery verifies the process tree's ports.
...
portboard url dev
portboard url dev api
portboard url dev --open
portboard logs dev --lines 200
portboard logs dev --follow
portboard stop dev
```
